...
if __name__ == "__main__":
    ...
    for price in range(0, len(index_prices)):

        current_price = index_prices[price]

        if dates[price].endswith("01"):
            if count != 0:
                monthly_index.append(index_value/count)
                monthly_dates.append(dates[price])
                index_value = 0
                count = 0
